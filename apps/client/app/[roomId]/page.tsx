...
const page = ({ params }: { params: { userId: string } }) => {
  ...
  const userState = useRecoilValue(userId);
  ...
  return (
    <>
      <div className="flex flex-col justify-center items-center h-screen overflow-x-hidden p-0">
        <div
          className="flex flex-col justify-end p-2 h-screen w-full text-black"
        ...
        >
          {server.map((messages, index) => (
            ...
            >
              <div
                className={`chat-bubble rounded-lg w-fit ${
                  messages.senderId === userState.id
          ...
          <div className="flex justify-between p-4">
            <input
              ...
              className="w-full bg-slate-900 border relative z-10 border-black focus:bg-slate-800 focus:outline-none text-white rounded-lg px-4 py-2"
            />
            <button
